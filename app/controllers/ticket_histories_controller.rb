class TicketHistoriesController < ApplicationController
  before_action :set_ticket_history, only: [:edit, :update]
  ...
  def update
    return redirect_to tickets_path, notice: 'Ticket history was successfully updated.' if @ticket_history.update_with_state(ticket_history_params)
    render action: 'edit'
  end
